...
int main(int, char**)
{
    ...
    world->setAmbient(Color(0.2));
    world->setSpecular(Color(1.0, 1.0, 1.0));
    world->setBackground(Color(0.1, 0.3, 0.6));
    
    Material *m = new Material(world);
    m->color = Color(0.7, 0.1, 0.0);
    m->ka = 0.6;
    m->kd = 0.9;
    m->ks = 0.3;
    m->n = 120;
    
    Object *sphere = new Sphere(Vector3D(-5, 0, -10), 3, m, world);
    world->addObject(sphere);
    
    Object *triangle = new Triangle(Vector3D(0, 0, -10),Vector3D(5, 0, -10),Vector3D(2.5, 4, -10), m, world);
    world->addObject(triangle); 

    //Object *sphere1 = new Sphere(Vector3D(-10, 0, -5), 3, m, world);
    //world->addObject(sphere1);
    Object *cylinder = new Cylinder(Vector3D(-5, 0, -10), 3, m, world);
    world->addObject(cylinder);

    LightSource *light = new PointLightSource(world, Vector3D(1, 1, 5), Color(0.2, 1, 0.2));
